...
import 'worker_category_screen.dart'; 
...
class _CommunityFeedScreenState extends State<CommunityFeedScreen> {
  ...
  Stream<List<PostModel>> _getActiveStream() {
    switch (_activeTab) {
      ...
      case 'Explore':
        return _postController.getPostsStream(categoryFilter: _exploreCategory);
      case 'My Category':
        return _postController.getSuggestedCategoryPostsStream(widget.userCategory ?? 'General');
      case 'My Uploads':
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        // 2. Add the gradient using flexibleSpace
        ...
        bottom: PreferredSize(
          ...
          child: Container(
            ...
            padding: const EdgeInsets.only(bottom: 16, top: 4, left: 12, right: 12), 
            child: FittedBox(
              ...
              child: Row(
                ...
                children: _tabs.map((tab) {
                  ...
                  String tabText = tab;
                  if (tab == 'Explore' && isSelected && _exploreCategory != null) {
                      ...
                      onTap: () async {
                        if (tab == 'Explore') {
                          ...
                          final selectedCategory = await Navigator.push(
                            context,
                            MaterialPageRoute(builder: (context) => const WorkerCategoryScreen()),
                          );
                          if (selectedCategory != null) {
                            setState(() => _exploreCategory = selectedCategory as String);
                          }
                        } else {
                          setState(() {
                            _activeTab = tab;
                      ...
                      child: AnimatedContainer(
                        ...
                        child: Text(
                          ...
                          style: TextStyle(
                            ...
                            color: isSelected ? const Color(0xFF4365FF) : Colors.white 
                          )
                        ),
                      ),
                    ),
      ...
      body: StreamBuilder<List<PostModel>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator(color: Color(0xFF4A90E2)));
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            ...
            return Center(
              child: Column(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  const Icon(Icons.dynamic_feed_rounded, size: 80, color: Color(0xFFCFD8DC)),
                  const SizedBox(height: 16),
                  Text(
                    _exploreCategory != null ? 'No posts for $_exploreCategory' : 'No posts in $_activeTab yet', 
                    style: const TextStyle(color: Color(0xFF78909C), fontSize: 18, fontWeight: FontWeight.bold)
                  ),
                  const SizedBox(height: 8),
                  const Text('Check back later for new content.', style: TextStyle(color: Color(0xFF90A4AE))),
                ],
              ),
            );
          }
              ...
              onPressed: () {
                Navigator.push(context, MaterialPageRoute(
                  builder: (context) => UploadPostScreen(
                    ...
                    category: widget.userCategory ?? 'General',
                  ),
                ));
              },
            )
