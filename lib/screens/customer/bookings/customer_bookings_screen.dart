...
_Cfg _cfg(String status) {
  switch (status) {
    case 'pending':            return const _Cfg('Waiting',           _C.accent,  Color(0xFFEEF2FF), Icons.hourglass_top_rounded,        1);
    case 'accepted':           return const _Cfg('Accepted',          _C.orange,  Color(0xFFFFF7ED), Icons.check_circle_outline_rounded,  2);
    case 'rejected':           return const _Cfg('Declined',          _C.red,     _C.redBg,          Icons.cancel_outlined,               1);
    case 'inprogress':         return const _Cfg('Paid',              _C.blue,    Color(0xFFEFF6FF), Icons.payment_rounded,               3);
    case 'arrived':            return const _Cfg('Worker Arrived',    _C.green,   Color(0xFFF0FDF4), Icons.directions_walk_rounded,       3);
    case 'completed':          return const _Cfg('Completed',         _C.green,   Color(0xFFECFDF5), Icons.verified_rounded,              4);
    // ── Quotation statuses ──
    case 'quotation_sent':     return const _Cfg('Quotation!',        _C.blue,    Color(0xFFEFF6FF), Icons.receipt_long_rounded,          4);
    case 'quotation_paid':     return const _Cfg('Quotation Paid',    _C.green,   Color(0xFFECFDF5), Icons.check_circle_rounded,          4);
    case 'quotation_declined': return const _Cfg('Qtn Declined',      _C.red,     _C.redBg,          Icons.cancel_outlined,               4);
    case 'cancelled':          return const _Cfg('Cancelled',         _C.muted,   Color(0xFFF4F6FA), Icons.do_not_disturb_rounded,        0);
    default:                   return const _Cfg('Submitted',         _C.accent,  Color(0xFFEEF2FF), Icons.radio_button_checked,          0);
  }
}

// Active statuses — includes quotation_sent so customer sees the alert
const _activeStatuses = [
  'pending', 'accepted', 'inprogress', 'arrived',
  'completed', 'quotation_sent', 'quotation_paid',
];
...
class _MyBookingsScreenState extends State<MyBookingsScreen>
  ...
  Widget _header() => Container(
        color: Colors.white,
        padding: EdgeInsets.only(top: MediaQuery.of(context).padding.top + 4, bottom: 0, left: 16, right: 16),
        child: Row(children: [
          ...
          const Text('My Bookings', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: _C.txt1)),
        ]),
      );
...
class _BookingListState extends State<_BookingList> {
  ...
  void _navigate(BuildContext context, Map<String, dynamic> d) {
    ...
    // ── Quotation sent → go to quotation view screen ──────────────────────────
    ...
    // ── All other active statuses → go to waiting worker screen ──────────────
    Navigator.push(context, MaterialPageRoute(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          if (hasQuotation) ...[
            Container(
              ...
              child: const Row(children: [
                Icon(Icons.notifications_rounded, size: 14, color: Colors.white),
                SizedBox(width: 8),
                Expanded(child: Text('New quotation received! Tap to review.',
            ...
            Container(
              ...
              decoration: BoxDecoration(
                gradient: (!isCancelled && !isRejected)
                    ...
                    : null,
                color: isRejected ? _C.redBg : isCancelled ? const Color(0xFFF0F2F8) : null,
                borderRadius: BorderRadius.circular(10),
              ),
              child: Icon(_catIcon(category), size: 14,
                  color: (isCancelled || isRejected) ? (isRejected ? _C.red : _C.muted) : Colors.white),
            ),
            const SizedBox(width: 10),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text('#$shortId', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: isCancelled ? _C.muted : _C.txt1)),
              Text(category, style: const TextStyle(fontSize: 10, color: _C.txt2)),
            ])),
            // Status badge
            Container(
          ...
          Row(children: [
            if (workerName != null && !isCancelled) ...[
              Container(width: 22, height: 22,
                  ...
                  child: Center(child: Text(workerName.isNotEmpty ? workerName[0].toUpperCase() : '?',
                      style: const TextStyle(color: Colors.white, fontSize: 9, fontWeight: FontWeight.w700)))),
              const SizedBox(width: 6),
              Expanded(child: Text(workerName,
                  style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w500, color: _C.txt1),
                  overflow: TextOverflow.ellipsis)),
            ] else
              const Expanded(child: Text('No worker assigned', style: TextStyle(fontSize: 10, color: _C.muted))),
            if (createdAt != null) ...[
              const SizedBox(width: 8),
              Text(_shortDate(createdAt), style: const TextStyle(fontSize: 10, color: _C.muted)),
            ],
            if (!isCancelled) ...[
              const SizedBox(width: 4),
              const Icon(Icons.chevron_right_rounded, size: 16, color: _C.muted),
            ],
          ]),

          // ── Worker location chip ────────────────────────────────────────────
          if (status == 'accepted' || status == 'inprogress' || status == 'arrived')
          ...
          if (isRejected) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(color: _C.redBg, borderRadius: BorderRadius.circular(8)),
              child: const Text('Worker declined this request',
                  style: TextStyle(fontSize: 10, color: Color(0xFFB91C1C), fontWeight: FontWeight.w500)),
            ),
          ],
          ...
          if (hasQuotation) ...[
            ...
            Container(
              ...
              child: Row(children: [
                const Icon(Icons.receipt_long_rounded, size: 12, color: _C.blue),
                const SizedBox(width: 6),
                Expanded(child: Text(
                  data['quotationJobDesc'] as String? ?? 'New quotation from worker',
                  style: const TextStyle(fontSize: 10, color: _C.blue),
                  maxLines: 1, overflow: TextOverflow.ellipsis,
                )),
                const SizedBox(width: 8),
                if (data['quotationTotalCost'] != null)
                  Text('LKR ${(data['quotationTotalCost'] as num).toStringAsFixed(0)}',
          ...
          return Expanded(child: Row(children: [
            Container(width: 16, height: 16,
              ...
              child: Center(child: isDone
                  ...
                  : isRejStep ? const Icon(Icons.close_rounded, size: 8, color: Colors.white)
                  : null)),
            if (i < 4) Expanded(child: Container(height: 2,
                decoration: BoxDecoration(
                  gradient: isDone && !isRejected ? const LinearGradient(colors: [_C.gradA, _C.gradB]) : null,
                  color: (isDone && !isRejected) ? null : const Color(0xFFE2E6F0),
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        final distTxt = (km > 100 || custLat == 0) ? 'Worker is nearby' : km < 1 ? '${(km * 1000).round()} m away' : '${km.toStringAsFixed(1)} km away';
        return Padding(
          padding: const EdgeInsets.only(top: 8),
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 6),
            decoration: BoxDecoration(color: const Color(0xFFEFF6FF), borderRadius: BorderRadius.circular(8), border: Border.all(color: const Color(0xFFBFDBFE), width: 0.5)),
            child: Row(children: [
              const Icon(Icons.location_on_rounded, size: 12, color: _C.blue), const SizedBox(width: 4),
              Text('Worker is $distTxt', style: const TextStyle(fontSize: 10, color: _C.blue, fontWeight: FontWeight.w500)),
              const Spacer(), _PulseDot(),
            ]),
          ),
        );
      },
...
class _PulseDotState extends State<_PulseDot> with SingleTickerProviderStateMixin {
  ...
  @override void initState() { super.initState(); _c = AnimationController(vsync: this, duration: const Duration(milliseconds: 1000))..repeat(reverse: true); _a = Tween<double>(begin: 0.3, end: 1.0).animate(CurvedAnimation(parent: _c, curve: Curves.easeInOut)); }
  @override void dispose() { _c.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => AnimatedBuilder(animation: _a, builder: (_, __) => Container(width: 7, height: 7, decoration: BoxDecoration(color: _C.green.withOpacity(_a.value), shape: BoxShape.circle)));
}
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Container(width: 72, height: 72, decoration: const BoxDecoration(color: Color(0xFFEEF2FF), shape: BoxShape.circle),
            child: const Icon(Icons.receipt_long_outlined, size: 30, color: _C.accent)),
        const SizedBox(height: 14),
        Text(isActive ? 'No Active Bookings' : 'No Booking History',
            style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: _C.txt1)),
        const SizedBox(height: 6),
        Text(isActive ? 'Active inspection requests\nwill appear here.' : 'Completed and past requests\nwill appear here.',
            textAlign: TextAlign.center, style: const TextStyle(fontSize: 11, color: _C.txt2, height: 1.6)),
      ]));
}