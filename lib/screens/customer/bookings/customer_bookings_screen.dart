...
_Cfg _cfg(String status) {
  switch (status) {
    ...
    case 'inprogress': return const _Cfg('In Progress', _C.blue,    Color(0xFFEFF6FF), Icons.build_rounded,                  3);
...
// ════════════════════════════════════════════════════════════════════════════════
// Booking list
// FIX: Instead of whereIn + orderBy (needs composite index),
//      we query only by customerId + status individually and merge in Dart.
// ════════════════════════════════════════════════════════════════════════════════
...
class _BookingListState extends State<_BookingList> {
  // Active statuses
  static const _activeStatuses  = ['pending', 'accepted', 'inprogress'];
  // History statuses
  ...
  void _subscribeAll() {
    ...
    final statuses =
        widget.isActive ? _activeStatuses : _historyStatuses;

    // One simple query per status — no composite index needed
    for (final status in statuses) {
      final sub = FirebaseFirestore.instance
        ...
        if (!mounted) return;
        setState(() {
          // Remove old docs with this status, re-add fresh ones
  ...
  Widget build(BuildContext context) {
    ...
    if (_docs.isEmpty) {
      return _EmptyState(isActive: widget.isActive);
    }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Column(
          ...
          // ── Worker location chip (accepted / inprogress) ────────────────────
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<DocumentSnapshot>(
      ...
      builder: (context, snap) {
        ...
        var km        = _haversine(lat, lng, custLat, custLng);
        // Guard: if result is unreasonably large, show generic message
  ...
  Widget build(BuildContext context) => Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(
            ...
            style: const TextStyle(
                fontSize: 11, color: _C.txt2, height: 1.6),
          ),
        ]),
      );
}
