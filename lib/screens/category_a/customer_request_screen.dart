...
// ════════════════════════════════════════════════════════════════════════════
// STAGE MAP
// 0 = submitted  (brief flash)
// 1 = waiting    (pending / searching)
// 2 = accepted   (show bill + Pay Now)
// 3 = inprogress (payment done, live map)
// 4 = completed  (worker marked done)
// _rejected flag (stage stays 1 but shows red banner)
// ════════════════════════════════════════════════════════════════════════════
...
class _CustomerRequestScreenState extends State<CustomerRequestScreen>
  ...
  void _applyStatus(String status) {
    switch (status) {
      ...
      case 'completed':
        ...
        break;
      case 'cancelled':
        break; // handled by explicit action
    }
  }
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(children: [
                ...
                if (_stage == 4)               _buildCompletedCard(),
              ]),
            ),
          ),
          _buildBottomBar(),
        ...
        child: Column(children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: _rejected
                  ? Border.all(color: _C.redBdr, width: 0.5)
                  : null,
            ),
  ...
  Widget _buildSummaryCard() {
    ...
    final shortId  =
        (rid.length >= 8 ? rid.substring(0, 8) : rid).toUpperCase();

    String estLabel, estVal; Color estColor;
    String statusTxt;        Color statusColor;

    if (_rejected) {
      estLabel = 'Status';   estVal = 'Declined'; estColor = _C.red;
      statusTxt = 'Declined'; statusColor = _C.red;
    } else {
      switch (_stage) {
        case 1:
        ...
        case 4:
          estLabel = 'Work Status'; estVal = 'Completed'; estColor = _C.green;
  ...
  Widget _buildPaymentBadge() => Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14),
          border:
              Border.all(color: const Color(0xFFBBF7D0), width: 0.5),
        ),
        child: Row(children: [
          Container(
            width: 32, height: 32,
  ...
  Widget _buildMapSection() {
    ...
    if (_workerLatLng != null) {
      markers.add(Marker(
        ...
        infoWindow:
            const InfoWindow(title: 'Your Worker', snippet: 'Live'),
        icon: BitmapDescriptor.defaultMarkerWithHue(
          ...
          child: SizedBox(
            ...
            child: GoogleMap(
              initialCameraPosition: CameraPosition(
                target:
                    _workerLatLng ?? LatLng(custLat, custLng),
                zoom: 15,
              ),
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        const Text(
          ...
          textAlign: TextAlign.center,
          style: TextStyle(
              fontSize: 10, color: _C.txt2, height: 1.6),
        ),
        if (ts != null) ...[
          const SizedBox(height: 10),
          const Divider(color: _C.cardBdr),
          const SizedBox(height: 8),
  ...
  Widget _bottomContent() {
    if (_rejected) {
      return _gradBtn(
        label: 'Go Back',
        colors: [_C.red, const Color(0xFFDC2626)],
        // Goes back to inspection/worker selection screen
        onTap: () => Navigator.pop(context),
      );
    }
    switch (_stage) {
      case 2:
        return _gradBtn(
          ...
          colors: [const Color(0xFF27AE60), _C.greenDk],
          onTap: _openPaymentScreen,
        );
      case 3:
        return _infoBar(
          icon: Icons.info_outline_rounded,
          text: 'Waiting for worker to complete the inspection',
          color: _C.blue,
  ...
  // ── Navigate to temp payment screen ───────────────────────────────────────
  void _openPaymentScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => TempPaymentScreen(
    ...
    // Firestore listener auto-moves to stage 3 when 'inprogress' is written.
  }
  ...
  Future<void> _handleCancel() async {
    ...
    if (mounted) {
      // FIX: go back to the inspection/worker selection screen (not home)
...
// ════════════════════════════════════════════════════════════════════════════
// TEMP PAYMENT SCREEN
// Shown when user taps "Pay Now".
// "Payment Done" button writes status='inprogress' + paymentStatus='paid'
// then pops — the Firestore listener in CustomerRequestScreen auto-updates.
// Replace this screen with the real payment UI when middleware is ready.
// ════════════════════════════════════════════════════════════════════════════
...
class _TempPaymentScreenState extends State<TempPaymentScreen> {
  ...
  Future<void> _markPaymentDone() async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .doc(widget.requestId)
          .update({
        'status':        'inprogress',  // triggers stage 3 on customer screen
        'paymentStatus': 'paid',
        'paymentMethod': 'manual',
        'paidAt':        FieldValue.serverTimestamp(),
        'totalPaid':     widget.totalAmount,
      });
      if (mounted) Navigator.pop(context); // back to CustomerRequestScreen
    } catch (e) {
            ...
            child: Column(children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(18),
                  border:
                      Border.all(color: _C.cardBdr, width: 0.5),
                ),
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
  ...
  Widget _pRow(String label, double amount, {String? tag}) => Padding(
        ...
        child: Row(children: [
          Text(label,
              style:
                  const TextStyle(fontSize: 11, color: _C.txt2)),
          if (tag != null) ...[
            const SizedBox(width: 5),
            Container(
        ...
        builder: (_, __) => Transform.translate(
          ...
          child: Container(
            ...
            decoration: const BoxDecoration(
              shape: BoxShape.circle,
              gradient:
                  LinearGradient(colors: [_C.gradA, _C.gradB]),
            ),
          ),
        ),
      );
