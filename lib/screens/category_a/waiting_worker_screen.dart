...
import '../category_a/payment/payment_screen.dart'; // adjust path
...
// ════════════════════════════════════════════════════════════════════════════
// STAGE MAP
// 0 = submitted  → pending   → waiting
// 1 = waiting    → searching for worker
// 2 = accepted   → show bill + Pay Now
// 3 = inprogress → payment done, show live map tracking
// 4 = completed  → show full work summary
// _rejected      → stays stage 1, shows red banner
// ════════════════════════════════════════════════════════════════════════════
...
class _WaitingWorkerScreenState extends State<WaitingWorkerScreen>
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // BACKEND — Firestore real-time listener
  // Listens to requests/{requestId} and reacts to every status change.
  // ══════════════════════════════════════════════════════════════════════════
  ...
  void _applyStatus(String status) {
    switch (status) {
      ...
      case 'inprogress':
        _rejected = false;
        _stage = 3;
        _startWorkerTracking(); // start live map
        break;
      case 'completed':
        _rejected = false;
        _stage = 4;
        _workerLocSub?.cancel(); // stop tracking
        break;
      case 'cancelled':
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // BACKEND — Worker live location tracking
  // Reads workers/{workerId} → currentLat, currentLng
  // Worker app must write these fields periodically when status = inprogress
  // ══════════════════════════════════════════════════════════════════════════
  void _startWorkerTracking() {
    ...
    _workerLocSub = FirebaseFirestore.instance
      ...
      _mapController?.animateCamera(
          CameraUpdate.newLatLng(_workerLatLng!));
    });
  }
  ...
  // ── Bill helpers ───────────────────────────────────────────────────────────
  // Reads stored fees from Firestore (written by InspectionFormScreen)
  double get _inspectionFee =>
      (_data['inspectionFee'] as num?)?.toDouble() ?? 1500.0;

  double get _distanceFee {
    final stored = (_data['distanceFee'] as num?)
        ?? (_data['travelFee'] as num?);
  ...
  bool get _doPulse  => _stage <= 3 && !_rejected;
  bool get _showDots => _arcSpin;

  String get _stageTitle {
    if (_rejected) return 'Request Declined';
    const t = ['Submitted', 'Waiting for Worker…', 'Worker Accepted!',
  ...
  String get _stageSub {
    ...
    switch (_stage) {
      ...
      case 3: return 'Payment confirmed. Your worker is on the way!';
      case 4: return 'The inspection has been completed. Thank you!';
      default: return '';
    }
  }
  ...
  String get _badgeTxt {
    ...
    const l = ['● Submitted', '● Waiting', '● Accepted',
          ...
          Expanded(
            child: SingleChildScrollView(
              ...
              child: Column(children: [
                ...
                if (_stage == 4)                _buildCompletedSummary(),
              ]),
            ),
          ),
          _buildBottomBar(),
              ...
              AnimatedBuilder(
                ...
                builder: (_, __) => Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: _doPulse
                        ? [BoxShadow(
                            color: (_stage == 4 ? _C.green : _C.accent)
                                .withOpacity(0.35),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: List.generate(5, (i) {
          ...
          if (isRej) activeColor = _C.reject;
          else if (display == 4 && isActive) activeColor = _C.green;
          else if (display == 3 && isActive) activeColor = _C.blue;
            ...
            child: Column(
              ...
              Text('Request Declined',
                  style: TextStyle(fontSize: 12,
                      fontWeight: FontWeight.w600, color: _C.reject)),
              SizedBox(height: 2),
              Text(
                'The worker has declined your request. Go back and select another worker.',
                style: TextStyle(fontSize: 10,
                    color: Color(0xFFB91C1C), height: 1.5),
              ),
            ]),
          ),
        ]),
  ...
  Widget _buildSummaryCard() {
    ...
    if (_rejected) {
      estLabel = 'Status'; estVal = 'Declined'; estColor = _C.reject;
      statusTxt = 'Declined'; statusColor = _C.reject;
    } else {
      switch (_stage) {
        case 1:
          estLabel = 'Est. Response'; estVal = '~5–10 min';
          estColor = _C.accent;
          statusTxt = 'Pending'; statusColor = _C.accent; break;
        case 2:
          estLabel = 'Worker ETA'; estVal = '~8 min';
          estColor = _C.orange;
          statusTxt = 'Accepted'; statusColor = _C.orange; break;
        case 3:
          estLabel = 'Work Status'; estVal = 'In Progress';
          estColor = _C.blue;
          statusTxt = 'In Progress'; statusColor = _C.blue; break;
        case 4:
          estLabel = 'Work Status'; estVal = 'Completed';
          estColor = _C.green;
          statusTxt = 'Completed'; statusColor = _C.green; break;
        default:
          estLabel = 'Est. Response'; estVal = 'Calculating…';
          estColor = _C.accent;
    ...
    return _SumCard(
      ...
      rows: [
        ...
        _SumRow(label: 'Status',      value: statusTxt,
  ...
  Widget _buildBillCard() => Container(
        ...
        child: Column(
          ...
          Row(children: [
            ...
            Text(_stage >= 3 ? 'PAYMENT SUMMARY' : 'BILL SUMMARY',
                style: const TextStyle(fontSize: 11,
                    fontWeight: FontWeight.w500, color: _C.muted,
                    letterSpacing: 0.5)),
          ]),
          ...
          const Divider(
              height: 16, color: _C.cardBorder, thickness: 1.5),
          Row(children: [
            const Expanded(child: Text('Total',
                style: TextStyle(fontSize: 12,
                    fontWeight: FontWeight.w700, color: _C.txt1))),
              ...
              Container(
                ...
                child: const Text('Fixed',
                    style: TextStyle(fontSize: 8,
                        color: _C.green, fontWeight: FontWeight.w500)),
              ),
            ],
          ])),
  ...
  Widget _buildPaymentConfirmedBadge() => Container(
        ...
        child: Row(children: [
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('LKR ${_fmt(_totalAmount)} paid successfully',
                style: const TextStyle(
  ...
  // ── Live map section (stage 3) ─────────────────────────────────────────────
  Widget _buildMapSection() {
    final custLat =
        (_data['latitude']  as num?)?.toDouble() ?? 6.9271;
    final custLng =
        (_data['longitude'] as num?)?.toDouble() ?? 79.8612;
    ...
    if (_workerLatLng != null) {
      markers.add(Marker(
        ...
        infoWindow: const InfoWindow(
            title: 'Your Worker', snippet: 'Live location'),
        icon: BitmapDescriptor.defaultMarkerWithHue(
    ...
    return Container(
      ...
      child: Column(
        ...
        Row(children: [
          ...
          const Text('LIVE WORKER TRACKING',
              style: TextStyle(fontSize: 11,
                  fontWeight: FontWeight.w500, color: _C.muted,
                  letterSpacing: 0.5)),
          const Spacer(),
  ...
  Widget _buildCompletedSummary() {
    ...
    final category    = _data['category']     as String? ?? '—';
    final imageUrls   = (_data['imageUrls']   as List<dynamic>?)
    ...
    return Column(children: [
      ...
      Container(
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          const Text('Inspection Complete',
              style: TextStyle(fontSize: 15,
                  fontWeight: FontWeight.w700, color: _C.green)),
          const SizedBox(height: 4),
          Text('By $workerName  •  $category',
              style: const TextStyle(fontSize: 10, color: _C.txt2)),
          if (ts != null) ...[
      ...
      _detailCard('WORK DETAILS', [
        ...
        if (notes != null && notes.isNotEmpty)
          _SumRow(label: 'Notes', value: notes),
      ]),

      // Payment summary
      _detailCard('PAYMENT SUMMARY', [
        _SumRow(label: 'Inspection Fee',
            value: 'LKR ${_fmt(_inspectionFee)}'),
        _SumRow(label: 'Distance Fee',
            value: 'LKR ${_fmt(_distanceFee)}'),
        _SumRow(label: 'Service Fee',
            value: 'LKR ${_fmt(_serviceFee)}'),
        _SumRow(label: 'Total Paid',
            value: 'LKR ${_fmt(_totalAmount)}',
            valueColor: _C.green),
        if (paidAt != null)
          _SumRow(label: 'Paid At', value: _fmtTs(paidAt)),
        _SumRow(label: 'Payment',   value: 'Confirmed',
            valueColor: _C.green),
      ]),
  ...
  Widget _detailCard(String title, List<_SumRow> rows) =>
      Container(
        margin: const EdgeInsets.fromLTRB(16, 8, 16, 0),
        padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(
  ...
  String _fmtTs(Timestamp ts) {
    ...
    final h    = dt.hour > 12
        ? dt.hour - 12 : dt.hour == 0 ? 12 : dt.hour;
  ...
  Widget _bottomContent() {
    ...
    switch (_stage) {
      ...
      case 3:
        return Container(
          ...
          child: const Row(
            ...
            SizedBox(width: 6),
            Text('Waiting for worker to complete the job',
                style: TextStyle(fontSize: 12,
                    fontWeight: FontWeight.w500, color: _C.blue)),
          ]),
        );
      case 4:
        return _gradBtn(
      ...
      default:
        return _outlineBtn(
            label: 'Cancel Request', onTap: _handleCancel);
    }
  }

  // Opens the temp payment screen embedded below
  void _openPayment() {
  Navigator.push(
    context,
    MaterialPageRoute(
        ...
        child: Container(
          ...
          child: Center(child: Text(label,
              style: const TextStyle(fontSize: 13,
                  fontWeight: FontWeight.w500, color: _C.txt2))),
        ),
      );
}
...
// ════════════════════════════════════════════════════════════════════════════
// TEMP PAYMENT SCREEN
// Shown when customer taps "Pay Now".
// "Payment Done" writes status=inprogress + paymentStatus=paid to Firestore.
// WaitingWorkerScreen listener auto-moves to stage 3.
// Replace with real payment gateway when middleware is ready.
// ════════════════════════════════════════════════════════════════════════════
class _TempPaymentScreen extends StatefulWidget {
  final String requestId;
  ...
  @override
  State<_TempPaymentScreen> createState() => _TempPaymentScreenState();
}

class _TempPaymentScreenState extends State<_TempPaymentScreen> {
  bool _loading = false;
  ...
  // ── BACKEND: write payment confirmation to Firestore ──────────────────────
  Future<void> _markPaymentDone() async {
    ...
    try {
      await FirebaseFirestore.instance
          ...
          .doc(widget.requestId)
          .update({
        'status':        'inprogress', // → triggers stage 3
        'paymentStatus': 'paid',
        'paymentMethod': 'manual',
        'paidAt':        FieldValue.serverTimestamp(),
        'totalPaid':     widget.totalAmount,
      });
      if (mounted) Navigator.pop(context); // → back to WaitingWorkerScreen
    } catch (e) {
        ...
        body: Column(children: [
              ...
              child: Column(children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color: const Color(0xFFFED7AA), width: 0.5),
                  ),
                  child: const Row(children: [
                    Icon(Icons.info_outline_rounded,
                        size: 14, color: _C.orange),
                    SizedBox(width: 8),
                    Expanded(
                      child: Text(
                        'Complete your payment via the agreed method, then press "Payment Done" to confirm.',
                        style: TextStyle(fontSize: 10,
                            color: _C.orange, height: 1.5),
                      ),
                    ),
                  ]),
                ),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    _pRow('Inspection Fee',
                        widget.inspectionFee, tag: 'Fixed'),
                    _pRow('Distance Fee',   widget.distanceFee),
                    _pRow('Service Fee (5%)', widget.serviceFee),
                    const Divider(height: 20,
                        color: _C.cardBorder, thickness: 1.5),
                    Row(children: [
                      const Expanded(child: Text('Total',
          ...
          Container(
            ...
            child: GestureDetector(
              onTap: _loading ? null : _markPaymentDone,
              child: Container(
                width: double.infinity,
                padding: const EdgeInsets.symmetric(vertical: 15),
                decoration: BoxDecoration(
            ...
            Container(
              ...
              child: Text(tag,
                  style: const TextStyle(fontSize: 8,
                      color: _C.green, fontWeight: FontWeight.w600)),
            ),
          ],
          const Spacer(),
          Text('LKR ${_fmt(amount)}',
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (spinning) {
      canvas.drawArc(
          ...
          spinValue * 2 * math.pi, math.pi * 1.4, false, p);
    } else {
      canvas.drawArc(
          Rect.fromCircle(center: const Offset(cx, cy), radius: r),
          -math.pi / 2, 2 * math.pi * (progress ?? 1.0), false, p);
    }
  }
  @override
