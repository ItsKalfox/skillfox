...
// 0 = submitted/pending
// 1 = waiting
// 2 = accepted  → bill + Pay Now
// 3 = paid/inprogress → worker on way, live map
// 4 = arrived   → worker at location, timer
// 5 = completed → inspection done
// 6 = quotation_sent     → show View Quotation button
// 7 = quotation_paid     → quotation payment done
// 8 = quotation_declined → customer declined
...
class _WaitingWorkerScreenState extends State<WaitingWorkerScreen>
  ...
  void _startWorkerTracking() {
    ...
    _workerLocSub = FirebaseFirestore.instance.collection('workers').doc(workerId).snapshots().listen((snap) {
      if (!snap.exists || !mounted) return;
      final d   = snap.data()!;
  ...
  Future<void> _handleCancel() async {
    final ok = await showDialog<bool>(context: context, builder: (_) => AlertDialog(
      ...
      actions: [
        ...
        ElevatedButton(style: ElevatedButton.styleFrom(backgroundColor: _C.reject),
    ...
    try {
      await FirebaseFirestore.instance.collection('requests').doc(widget.requestId)
          .update({'status': 'cancelled', 'cancelledBy': 'customer', 'cancelledAt': FieldValue.serverTimestamp()});
    } catch (e) { debugPrint('Cancel error: $e'); }
    if (mounted) Navigator.pop(context);
  }

  // Bill
  double get _inspectionFee => (_data['inspectionFee'] as num?)?.toDouble() ?? 1500.0;
  ...
  double get _serviceFee  => (_data['serviceFee'] as num?)?.toDouble() ?? ((_inspectionFee + _distanceFee) * 0.05).roundToDouble();
  double get _totalAmount => (_data['totalAmount'] as num?)?.toDouble() ?? (_inspectionFee + _distanceFee + _serviceFee);
  ...
  String get _stageTitle {
    ...
    switch (_stage) {
      ...
      case 1: return 'Waiting for Worker…';
      case 2: return 'Worker Accepted!';
      case 3: return 'Worker On The Way';
      case 4: return 'Worker Has Arrived!';
      case 5: return 'Inspection Completed';
      case 6: return 'Quotation Received';
      case 7: return 'Quotation Paid';
      case 8: return 'Quotation Declined';
      case 9: return 'Job Completed!';
      default: return '';
    }
  }
  ...
  String get _stageSub {
    ...
    switch (_stage) {
      ...
      case 2: return '$name accepted your request. Review the bill and complete payment.';
      case 3: return 'Payment confirmed! $name is heading to your location.';
      case 4: return '$name has arrived at your location. Work has started!';
      case 5: return 'The inspection has been completed. A quotation is being prepared.';
      case 6: return '$name has sent you a quotation. Review and accept or decline it.';
      case 7: return 'Quotation payment confirmed. Work is in progress.';
      case 8: return 'You declined the quotation. Please leave a review.';
      case 9: return 'The quotation has been completed successfully!';
      default: return '';
    }
  }
  ...
  Color get _badgeColor {
    ...
    switch (_stage) {
      ...
      case 4: return const Color(0xFFF0FDF4);
      case 5: return const Color(0xFFECFDF5);
      case 6: return const Color(0xFFEFF6FF);
      case 7: return const Color(0xFFECFDF5);
      case 8: return _C.rejectBg;
  ...
  Color get _badgeTxtColor {
    ...
    switch (_stage) {
      ...
      case 6: return _C.blue;
      case 7: return _C.green;
      case 8: return _C.reject;
  ...
  String get _badgeTxt {
    ...
    switch (_stage) {
      ...
      case 3: return '● Worker On Way';
      case 4: return '● Worker Arrived';
  ...
  IconData get _blobIcon {
    ...
    switch (_stage) {
      ...
      case 5: return Icons.verified_rounded;
      case 6: return Icons.receipt_long_rounded;
      case 7: return Icons.check_circle_rounded;
      case 8: return Icons.cancel_outlined;
  ...
  Widget _buildHeader() => Container(
        ...
        child: Row(children: [
          ...
          Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
              ...
              child: Text('#${_requestId.length >= 6 ? _requestId.substring(0, 6).toUpperCase() : _requestId.toUpperCase()}',
                  style: const TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: _C.accent))),
        ]),
      );
        ...
        child: Column(children: [
          SizedBox(width: 100, height: 100, child: Stack(alignment: Alignment.center, children: [
            AnimatedBuilder(animation: _spinCtrl, builder: (_, __) => CustomPaint(
              ...
              painter: _ArcPainter(progress: _arcSpin ? null : (_stage / 8.0), spinning: _arcSpin, spinValue: _spinCtrl.value, rejected: _rejected, completed: _stage >= 5),
            )),
            AnimatedBuilder(animation: _pulseAnim, builder: (_, __) => Container(
              width: 58, height: 58,
              decoration: BoxDecoration(shape: BoxShape.circle,
                gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight,
                  colors: _rejected || _stage == 8 ? [_C.reject, const Color(0xFFDC2626)] : _stage >= 5 ? [_C.green, _C.greenDark] : [_C.gradA, _C.gradB]),
                boxShadow: _doPulse ? [BoxShadow(color: (_stage >= 5 ? _C.green : _C.accent).withOpacity(0.35), blurRadius: _pulseAnim.value, spreadRadius: _pulseAnim.value * 0.3)] : []),
          ...
          Container(
            ...
            decoration: BoxDecoration(color: _badgeColor, borderRadius: BorderRadius.circular(20),
                border: (_rejected || _stage == 8) ? Border.all(color: _C.rejectBdr, width: 0.5) : null),
    ...
    return Padding(
      ...
      child: Row(mainAxisAlignment: MainAxisAlignment.center,
        children: List.generate(5, (i) {
          final isDone = i < display; final isActive = i == display; final isRej = _rejected && i == 1;
          return Row(mainAxisSize: MainAxisSize.min, children: [
            Column(children: [
              AnimatedContainer(duration: const Duration(milliseconds: 300), width: 24, height: 24,
                decoration: BoxDecoration(shape: BoxShape.circle,
                  gradient: (isDone || isActive) && !isRej ? const LinearGradient(colors: [_C.gradA, _C.gradB]) : null,
                  color: isRej ? _C.reject : (isDone || isActive) ? null : const Color(0xFFE8EAF0)),
                child: Center(child: isDone ? const Icon(Icons.check_rounded, size: 12, color: Colors.white)
                    : isRej ? const Icon(Icons.close_rounded, size: 12, color: Colors.white)
              ...
              SizedBox(width: 42, child: Text(labels[i], textAlign: TextAlign.center,
                  style: TextStyle(fontSize: 7.5, color: isRej ? _C.reject : isDone ? _C.gradA : isActive ? _C.accent : _C.muted,
  ...
  Widget _buildRejectionBanner() => Container(
        ...
        decoration: BoxDecoration(color: _C.rejectBg, borderRadius: BorderRadius.circular(14), border: Border.all(color: _C.rejectBdr)),
        child: Row(children: [
          Container(width: 36, height: 36, decoration: BoxDecoration(color: _C.reject.withOpacity(0.12), shape: BoxShape.circle),
              child: const Icon(Icons.cancel_outlined, size: 18, color: _C.reject)),
          const SizedBox(width: 12),
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Request Declined', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: _C.reject)),
            SizedBox(height: 2),
  ...
  Widget _buildArrivedBanner() => Container(
        ...
        child: Row(children: [
          ...
          const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            SizedBox(height: 2),
            Text('The worker is at your location. Inspection has started.', style: TextStyle(fontSize: 10, color: Color(0xFF166534), height: 1.5)),
          ])),
        ]),
      );

  // ── NEW: Quotation received card ──────────────────────────────────────────
  Widget _buildQuotationReceivedCard() {
    ...
    return Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: _C.cardBorder, width: 0.5)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Row(children: [
            Icon(Icons.receipt_long_rounded, size: 16, color: _C.blue), SizedBox(width: 8),
            Text('NEW QUOTATION', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: _C.muted, letterSpacing: 0.5)),
          ]),
          const SizedBox(height: 12),
          ...
          if (compTime.isNotEmpty) Row(children: [
            const Text('Est. Completion:', style: TextStyle(fontSize: 11, color: _C.txt2)), const SizedBox(width: 8),
            Text(compTime, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: _C.txt1)),
          ]),
          if (compTime.isNotEmpty) const SizedBox(height: 10),
          Container(padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(gradient: const LinearGradient(colors: [_C.gradA, _C.gradB]), borderRadius: BorderRadius.circular(10)),
              child: Row(children: [
                const Expanded(child: Text('Total Amount', style: TextStyle(fontSize: 12, color: Colors.white70))),
                Text('LKR ${_fmt(price)}', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: Colors.white)),
              ])),
        ]));
  }

  // ── NEW: Quotation paid card ──────────────────────────────────────────────
  Widget _buildQuotationPaidCard() {
    ...
    return Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(16),
        ...
        child: Column(children: [
          ...
          const SizedBox(height: 8),
          const Text('Quotation Payment Done!', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _C.green)),
          const SizedBox(height: 4),
          Text('LKR ${_fmt(price)} paid successfully', style: const TextStyle(fontSize: 12, color: _C.txt2)),
          const SizedBox(height: 8),
          const Text('Work is now in progress. You will be notified when done.', textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: _C.muted, height: 1.5)),
        ]));
  }

  // ── NEW: Quotation declined card ──────────────────────────────────────────
  Widget _buildQuotationDeclinedCard() => Container(
        margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(color: _C.rejectBg, borderRadius: BorderRadius.circular(18), border: Border.all(color: _C.rejectBdr)),
        child: Column(children: [
          const Icon(Icons.cancel_outlined, size: 44, color: _C.reject),
          const SizedBox(height: 8),
          const Text('Inspection was completed', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: _C.reject)),
          const SizedBox(height: 4),
          const Text('You declined the quotation. The inspection has been completed.\nWould you like to leave a review?',
              textAlign: TextAlign.center, style: TextStyle(fontSize: 10, color: Color(0xFFB91C1C), height: 1.5)),
        ]));

  Widget _buildSummaryCard() {
    final category = _data['category'] as String? ?? '—';
    final address  = _data['address']  as String? ?? '—';
    final shortId  = _requestId.length >= 8 ? _requestId.substring(0, 8).toUpperCase() : _requestId.toUpperCase();
    String estLabel = 'Est. Response', estVal = 'Calculating…'; Color estColor = _C.accent;
    String statusTxt = 'Submitted'; Color statusColor = _C.accent;
    if (_rejected) {
      estLabel = 'Status'; estVal = 'Declined'; estColor = _C.reject; statusTxt = 'Declined'; statusColor = _C.reject;
    } else {
      switch (_stage) {
        ...
        case 3: estLabel = 'Work Status'; estVal = 'On The Way'; estColor = _C.blue; statusTxt = 'Paid'; statusColor = _C.blue; break;
        case 4: estLabel = 'Work Status'; estVal = 'In Progress'; estColor = _C.orange; statusTxt = 'Arrived'; statusColor = _C.orange; break;
        case 5: estLabel = 'Work Status'; estVal = 'Completed'; estColor = _C.green; statusTxt = 'Completed'; statusColor = _C.green; break;
        case 6: estLabel = 'Work Status'; estVal = 'Quotation Sent'; estColor = _C.blue; statusTxt = 'Quotation'; statusColor = _C.blue; break;
        case 7: estLabel = 'Work Status'; estVal = 'Quotation Paid'; estColor = _C.green; statusTxt = 'Paid'; statusColor = _C.green; break;
        case 8: estLabel = 'Work Status'; estVal = 'Declined'; estColor = _C.reject; statusTxt = 'Declined'; statusColor = _C.reject; break;
      }
    }
    return _SumCard(title: _stage >= 3 ? 'SERVICE SUMMARY' : 'REQUEST SUMMARY', rows: [
  ...
  Widget _buildBillCard() => Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: _C.cardBorder, width: 0.5)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
  ...
  Widget _billRow(String label, double amount, {bool isFixed = false}) => Padding(
        padding: const EdgeInsets.symmetric(vertical: 5),
        child: Row(children: [
          Expanded(child: Row(children: [
            Text(label, style: const TextStyle(fontSize: 11, color: _C.txt2)),
            if (isFixed) ...[const SizedBox(width: 4), Container(padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1),
                decoration: BoxDecoration(color: const Color(0xFFECFDF5), borderRadius: BorderRadius.circular(4)),
                child: const Text('Fixed', style: TextStyle(fontSize: 8, color: _C.green, fontWeight: FontWeight.w500)))],
          ])),
          Text('LKR ${_fmt(amount)}', style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: _C.txt1)),
        ]),
      );

  Widget _buildPaymentConfirmedBadge() => Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0),
  ...
  Widget _buildCustomerTimerCard() => Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            const Text('INSPECTION IN PROGRESS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: _C.muted, letterSpacing: 0.5)),
          ]),
          ...
          const SizedBox(height: 8),
          const Center(child: Text('Time elapsed since worker arrived', style: TextStyle(fontSize: 10, color: _C.muted))),
        ]),
      );
    ...
    return Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(14),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          const SizedBox(width: 8),
          Text(_stage == 4 ? 'WORKER LOCATION' : 'LIVE WORKER TRACKING',
              style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: _C.muted, letterSpacing: 0.5)),
          const Spacer(), _LiveDot(),
        ]),
        const SizedBox(height: 10),
        ClipRRect(borderRadius: BorderRadius.circular(14), child: SizedBox(height: 240,
            child: GoogleMap(
              ...
              markers: markers,
              polylines: _workerLatLng != null && _stage == 3 ? {Polyline(polylineId: const PolylineId('route'), points: [_workerLatLng!, LatLng(custLat, custLng)], color: _C.accent, width: 3, patterns: [PatternItem.dash(20), PatternItem.gap(10)])} : {},
  ...
  Widget _dotLegend(Color color, String label) => Row(mainAxisSize: MainAxisSize.min, children: [
        ...
        const SizedBox(width: 5), Text(label, style: const TextStyle(fontSize: 10, color: _C.txt2)),
      ]);
  ...
  Widget _buildCompletedSummary() {
    ...
    final imageUrls = (_data['imageUrls'] as List<dynamic>?)?.map((e) => e.toString()).toList() ?? [];
    String durationStr = '';
    if (jobDuration != null) { final m = jobDuration ~/ 60; final s = jobDuration % 60; durationStr = '${m}m ${s}s'; }

    return Column(children: [
      Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(16),
        decoration: BoxDecoration(gradient: LinearGradient(begin: Alignment.topLeft, end: Alignment.bottomRight, colors: [_C.green.withOpacity(0.08), _C.gradB.withOpacity(0.05)]),
            borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFFBBF7D0), width: 0.5)),
        child: Column(children: [
          const Icon(Icons.verified_rounded, size: 44, color: _C.green),
          const SizedBox(height: 8),
          const Text('Inspection Complete', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: _C.green)),
          const SizedBox(height: 4),
          Text('By $workerName  •  $category', style: const TextStyle(fontSize: 10, color: _C.txt2)),
          if (durationStr.isNotEmpty) ...[const SizedBox(height: 4), Text('Duration: $durationStr', style: const TextStyle(fontSize: 10, color: _C.muted))],
          if (ts != null) ...[const SizedBox(height: 6), Text(_fmtTs(ts), style: const TextStyle(fontSize: 9, color: _C.muted))],
        ])),
      ...
      _detailCard('PAYMENT SUMMARY', [
        _SumRow(label: 'Inspection Fee', value: 'LKR ${_fmt(_inspectionFee)}'),
        _SumRow(label: 'Distance Fee',   value: 'LKR ${_fmt(_distanceFee)}'),
        _SumRow(label: 'Service Fee',    value: 'LKR ${_fmt(_serviceFee)}'),
        _SumRow(label: 'Total Paid',     value: 'LKR ${_fmt(_totalAmount)}', valueColor: _C.green),
        if (paidAt != null) _SumRow(label: 'Paid At', value: _fmtTs(paidAt)),
      ]),
      if (imageUrls.isNotEmpty)
        Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(14),
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: _C.cardBorder, width: 0.5)),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('INSPECTION PHOTOS', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: _C.muted, letterSpacing: 0.5)),
            const SizedBox(height: 10),
            GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: imageUrls.length,
                ...
                itemBuilder: (_, i) => ClipRRect(borderRadius: BorderRadius.circular(8), child: Image.network(imageUrls[i], fit: BoxFit.cover))),
          ])),
    ]);
  }

  Widget _detailCard(String title, List<_SumRow> rows) => Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(14),
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: _C.cardBorder, width: 0.5)),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(title, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: _C.muted, letterSpacing: 0.5)),
          const SizedBox(height: 10),
          ...rows.map((r) => Padding(padding: const EdgeInsets.symmetric(vertical: 5),
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                SizedBox(width: 100, child: Text(r.label, style: const TextStyle(fontSize: 11, color: _C.txt2))),
                Expanded(child: Text(r.value, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: r.valueColor ?? _C.txt1))),
              ]))),
        ]));
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(width: 28, height: 28,
              decoration: BoxDecoration(
                  gradient: const LinearGradient(colors: [Color(0xFF16A34A), Color(0xFF1E8449)]),
                  borderRadius: BorderRadius.circular(8)),
              child: const Icon(Icons.celebration_rounded, size: 14, color: Colors.white)),
          const SizedBox(width: 8),
          const Text('QUOTATION SUCCESSFULLY COMPLETED!',
              style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Color(0xFF16A34A), letterSpacing: 0.3)),
        ]),
        const SizedBox(height: 12),
        Container(
          padding: const EdgeInsets.all(12),
          ...
          child: const Row(children: [
            ...
            Expanded(child: Text(
                ...
                style: TextStyle(fontSize: 11, color: Color(0xFF16A34A), height: 1.5))),
          ]),
        ),
        const SizedBox(height: 12),
        const Text('Please take a moment to leave a review for the worker.',
            style: TextStyle(fontSize: 11, color: Color(0xFF888888))),
      ]),
    );
  }

  Widget _buildBottomBar() => Container(color: Colors.white,
        padding: EdgeInsets.fromLTRB(16, 14, 16, MediaQuery.of(context).padding.bottom + 20),
        child: _bottomContent());

  Widget _bottomContent() {
    if (_rejected) return _gradBtn(label: 'Go Back', colors: [_C.reject, const Color(0xFFDC2626)], onTap: () => Navigator.pop(context));
    switch (_stage) {
      case 2:
        return _gradBtn(label: 'Pay Now  —  LKR ${_fmt(_totalAmount)}', colors: [_C.gradA, _C.gradB], onTap: _openPayment);
      case 3:
        return _infoBar(icon: Icons.directions_car_rounded, text: 'Worker is on the way to your location', color: _C.blue, bg: const Color(0xFFEFF6FF), border: const Color(0xFFBFDBFE));
      case 4:
        return _infoBar(icon: Icons.build_rounded, text: 'Worker is performing the inspection', color: _C.orange, bg: const Color(0xFFFFF7ED), border: const Color(0xFFFED7AA));
      case 5:
        return _infoBar(icon: Icons.receipt_long_rounded, text: 'Inspection done. Waiting for quotation from worker...', color: _C.blue, bg: const Color(0xFFEFF6FF), border: const Color(0xFFBFDBFE));
      case 6:
        ...
        return _gradBtn(
          ...
          onTap: () => Navigator.push(context, MaterialPageRoute(
              builder: (_) => CustomerQuotationViewScreen(requestId: _requestId, requestData: _data))),
        );
      case 7:
        return _infoBar(icon: Icons.hourglass_top_rounded, text: 'Payment confirmed! Waiting for worker to complete the job...', color: _C.green, bg: const Color(0xFFECFDF5), border: const Color(0xFFBBF7D0));
      case 9:
        return _gradBtn(
          label: 'Leave a Review',
          colors: [_C.gradA, _C.gradB],
          onTap: () => Navigator.push(context, MaterialPageRoute(
              builder: (_) => ReviewScreen(requestId: _requestId, requestData: _data, isWorker: false))),
        );
      case 8:
        // Declined → go to review
        return _gradBtn(
          label: 'Leave a Review',
          colors: [_C.gradA, _C.gradB],
          onTap: () => Navigator.push(context, MaterialPageRoute(
  ...
  void _openPayment() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => PaymentScreen(
      requestId: _requestId, totalAmount: _totalAmount, inspectionFee: _inspectionFee, distanceFee: _distanceFee, serviceFee: _serviceFee,
    )));
  }

  Widget _gradBtn({required String label, required List<Color> colors, required VoidCallback onTap}) => GestureDetector(onTap: onTap,
  ...
  Widget _outlineBtn({required String label, required VoidCallback onTap}) => GestureDetector(onTap: onTap,
        child: Container(width: double.infinity, padding: const EdgeInsets.symmetric(vertical: 15),
            decoration: BoxDecoration(color: Colors.white, border: Border.all(color: _C.cardBorder, width: 1.5), borderRadius: BorderRadius.circular(14)),
...
class _SumCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(margin: const EdgeInsets.fromLTRB(16, 8, 16, 0), padding: const EdgeInsets.all(14),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          ...rows.map((r) => Padding(padding: const EdgeInsets.symmetric(vertical: 5),
              child: Row(children: [Expanded(child: Text(r.label, style: const TextStyle(fontSize: 11, color: _C.txt2))), Text(r.value, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: r.valueColor ?? _C.txt1))]))),
        ]));
}
