...
class _QuotationScreenState extends State<QuotationScreen> {
  ...
  Future<void> _sendQuotation() async {
    if (_jobDescCtrl.text.trim().isEmpty) { _showSnack('Please enter job description.', isError: true); return; }
    if (_labourCostCtrl.text.trim().isEmpty) { _showSnack('Please enter labour cost.', isError: true); return; }

    final labour   = double.tryParse(_labourCostCtrl.text.trim())   ?? 0;
    final material = double.tryParse(_materialCostCtrl.text.trim()) ?? 0;
    if (labour <= 0) { _showSnack('Please enter a valid labour cost.', isError: true); return; }
    ...
    try {
      await FirebaseFirestore.instance
        ...
        'quotationSent':           true,
        'quotationStatus':         'pending', // pending / accepted / declined
  ...
  Widget build(BuildContext context) {
    final custName = widget.requestData['customerName'] as String? ?? 'Customer';
      ...
      body: Column(children: [
        ...
        Container(
          ...
          child: Row(children: [
            ...
            const Text('Request Quotation', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: Colors.white)),
            const Spacer(),
            Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), borderRadius: BorderRadius.circular(12)),
                child: const Text('WORKER', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 0.5))),
          ]),
        ),
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Container(
                ...
                child: const Row(children: [
                  Icon(Icons.info_outline_rounded, size: 16, color: _C.orange),
                  SizedBox(width: 10),
                  Expanded(child: Text(
                    'Fill in the quotation details. This will be sent to the customer for approval before proceeding.',
                    style: TextStyle(fontSize: 11, color: _C.orange, height: 1.5),
                  )),
                ]),
              ),
              ...
              _textArea(_jobDescCtrl, 'Replaced leaking pipe joint under kitchen sink.\nCleaned drainage. Checked all visible connections.', 4),
              ...
              const SizedBox(height: 8),
              _textInput(_labourCostCtrl, '2500', TextInputType.number, onChanged: (_) => setState(() {})),
              ...
              const SizedBox(height: 8),
              _textInput(_materialCostCtrl, '1800', TextInputType.number, onChanged: (_) => setState(() {})),
              ...
              if (_totalCost > 0) ...[
                ...
                Container(
                  ...
                  child: Row(children: [
                    const Text('Total Cost:', style: TextStyle(fontSize: 12, color: Colors.white70)),
                    const Spacer(),
                    Text('LKR ${_fmt(_totalCost)}', style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: Colors.white)),
                  ]),
                ),
              ],
              ...
              const SizedBox(height: 8),
              _textInput(_completionTimeCtrl, '2 hours', TextInputType.text),
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: Colors.white,
                    borderRadius: BorderRadius.circular(14),
                    border: Border.all(color: _C.cardBdr, style: BorderStyle.solid, width: 1.5),
                  ),
                  child: _evidencePhoto != null
                      ? ClipRRect(
                          borderRadius: BorderRadius.circular(12),
                          child: Image.file(_evidencePhoto!, height: 120, fit: BoxFit.cover))
                      : Column(children: [
                          Icon(Icons.camera_alt_outlined, size: 32, color: Colors.grey.shade400),
                          const SizedBox(height: 8),
                          RichText(text: TextSpan(children: [
                            const TextSpan(text: 'Tap to upload ', style: TextStyle(fontSize: 12, color: _C.gradA, fontWeight: FontWeight.w600)),
                            TextSpan(text: 'a photo of the work area', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                          ])),
                          const SizedBox(height: 4),
                          Text('JPG, PNG up to 10MB', style: TextStyle(fontSize: 10, color: Colors.grey.shade400)),
                        ]),
                ),
              ),
              ...
              const SizedBox(height: 8),
              _textArea(_notesCtrl, 'Any additional notes for the customer...', 3),
          ...
          child: Column(children: [
              ...
              child: Container(
                ...
                child: Center(child: _loading
                    ...
                    : const Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        SizedBox(width: 8),
                        Text('Send Quotation to Customer', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w700)),
                      ])),
              ),
            ),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                child: const Center(child: Text('Cancel', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: _C.txt2))),
              ),
            ),
          ]),
        ),
  ...
  Widget _fieldLabel(String label) => Text(label,
      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: _C.txt1));

  Widget _textInput(TextEditingController ctrl, String hint, TextInputType type, {void Function(String)? onChanged}) =>
      Container(
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12), border: Border.all(color: _C.cardBdr)),
        child: TextField(
          controller: ctrl,
          keyboardType: type,
          onChanged: onChanged,
