...
class PostController {
  ...
  Stream<List<PostModel>> getForYouPostsStream() {
    return _firestore.collection('posts').snapshots().map((snapshot) {
      List<PostModel> posts = snapshot.docs
          ...
          .toList();
      posts.sort(
          (a, b) => _calculateForYouScore(b).compareTo(_calculateForYouScore(a)));
      return posts;
    });
  }
  ...
  double _calculateForYouScore(PostModel post) {
    ...
    final engagement = (post.likedBy.length * 1.0) +
        (post.commentCount * 2.0) +
        (post.savedBy.length * 3.0);
  ...
  Stream<List<PostModel>> getSuggestedCategoryPostsStream(String category) {
    return _firestore
      ...
      List<PostModel> posts = snapshot.docs
          ...
          .toList();
      posts.sort((a, b) =>
          _calculateAdvancedScore(b).compareTo(_calculateAdvancedScore(a)));
      return posts;
    });
  }
  ...
  double _calculateAdvancedScore(PostModel post) {
    ...
    final points = (post.likedBy.length * 2) +
        (post.commentCount * 3) +
        (post.savedBy.length * 4);
  ...
  Future<void> toggleLike(
      String postId, String currentUserId, List<dynamic> currentLikes) async {
  ...
  Future<void> toggleSave(
      String postId, String currentUserId, List<dynamic> currentSaves) async {
