...
class _WorkerRequestsScreenState extends State<WorkerRequestsScreen> {
  ...
  bool _isSubscriptionRequest(Map<String, dynamic> data) {
  ...
  String _subscriptionDays(Map<String, dynamic> data) {
    final raw = data['subscriptionDays'];
    if (raw is! List) return 'Not specified';
    final days = raw
  ...
  String _subscriptionCycleLabel(Map<String, dynamic> data) {
    final model = (data['subscriptionModel'] ?? '').toString().toLowerCase();
    final idx = (data['subscriptionCycleIndex'] as num?)?.toInt() ?? 1;
    if (model == 'monthly') return 'Month $idx';
    return 'Week $idx';
  }

  String _paymentState(Map<String, dynamic> data) {
    final payment = (data['subscriptionCyclePaymentStatus'] ?? '')
        .toString()
        .toLowerCase();
    return payment == 'paid' ? 'PAID' : 'PAYMENT PENDING';
  }

  String _cycleDateRange(Map<String, dynamic> data) {
    final ts = data['subscriptionCycleStartAt'] as Timestamp?;
    if (ts == null) return '';
                      ...
                      builder: (context, snapshot) {
                        ...
                        return ListView.builder(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final studentRef =
                                (data['studentRef'] ??
                                        data['acceptedStudentRef'] ??
                                        '')
                                    .toString();
                            final subscriptionModel =
                                (data['subscriptionModel'] ?? '').toString();
                            final daysLabel = _subscriptionDays(data);
                            final cycleLabel = _subscriptionCycleLabel(data);
                            final paymentState = _paymentState(data);
                            final cycleDate = _cycleDateRange(data);
                            final shortId =
                                '#' +
                                (doc.id.length >= 8
                                  ...
                                  if (isSubscription) ...[
                                    ...
                                    Container(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                          Text(
                                            'Student: ${studentRef.isEmpty ? studentName : '$studentName ($studentRef)'}',
                                            style: const TextStyle(
                                              fontSize: 11,
                                              fontWeight: FontWeight.w600,
