...
class _BookingCard extends StatelessWidget {
  ...
  bool _isSubscriptionRequest(Map<String, dynamic> d) {
    final requestType = (d['requestType'] ?? '').toString().toLowerCase();
  ...
  String _cycleDate(Map<String, dynamic> d) {
    ...
    String f(DateTime dt) => '${dt.day} ${months[dt.month - 1]} ${dt.year}';
    return '${f(start)} - ${f(end)}';
  }

  String _paymentState(Map<String, dynamic> d) {
    final payment = (d['subscriptionCyclePaymentStatus'] ?? '')
        .toString()
        .toLowerCase();
    return payment == 'paid' ? 'Paid' : 'Payment Pending';
  }

  Future<void> _payCurrentCycle(
    ...
    await FirebaseFirestore.instance.collection('requests').doc(id).update({
      'status': 'inprogress',
      'subscriptionCyclePaymentStatus': 'paid',
      'subscriptionCyclePaidAt': FieldValue.serverTimestamp(),
    });
    if (!context.mounted) return;
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Cycle payment completed.')));
  }
  ...
  Future<void> _startNextCycle(
    ...
    final id = d['id'] as String?;
    if (id == null || id.isEmpty) return;
    final current = (d['subscriptionCycleIndex'] as num?)?.toInt() ?? 1;
    await FirebaseFirestore.instance.collection('requests').doc(id).update({
      'status': 'accepted',
      'subscriptionCycleIndex': current + 1,
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Next cycle started. Please complete payment.'),
      ),
    );
  }

  Future<void> _endSubscription(
    BuildContext context,
    Map<String, dynamic> d,
  ) async {
    final id = d['id'] as String?;
    ...
    await FirebaseFirestore.instance.collection('requests').doc(id).update({
      'status': 'cancelled',
      'subscriptionEndedBy': 'customer',
      'subscriptionEndedAt': FieldValue.serverTimestamp(),
    });
    if (!context.mounted) return;
    ScaffoldMessenger.of(
      context,
    ).showSnackBar(const SnackBar(content: Text('Subscription ended.')));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final isSubscription = _isSubscriptionRequest(data);
    final cycleLabel = _cycleLabel(data);
    final cycleDate = _cycleDate(data);
    final cyclePaymentState = _paymentState(data);
    final paymentPending =
        (data['subscriptionCyclePaymentStatus'] ?? 'pending')
            .toString()
            .toLowerCase() !=
        'paid';
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            if (isSubscription && !isCancelled && !isRejected) ...[
              const SizedBox(height: 10),
              Row(
                children: [
                  if (status == 'accepted' && paymentPending)
                    Expanded(
                      child: OutlinedButton(
                        onPressed: () => _payCurrentCycle(context, data),
                  ...
                  if (status == 'completed')
                    Expanded(
                      child: OutlinedButton(
                        onPressed: () => _startNextCycle(context, data),
                        style: OutlinedButton.styleFrom(
                          side: const BorderSide(color: _C.blue),
                          foregroundColor: _C.blue,
                        ),
                  ...
                  Expanded(
                    child: OutlinedButton(
                      onPressed: () => _endSubscription(context, data),
                      style: OutlinedButton.styleFrom(
