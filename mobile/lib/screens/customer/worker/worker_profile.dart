...
import 'package:skillfox/screens/category_a/subscription_request_screen.dart';
...
class _WorkerProfileScreenState extends State<WorkerProfileScreen> {
  ...
  bool _isSubscriptionCategoryWorker() {
    const subscriptionCategories = {'teacher', 'caregiver'};
    return subscriptionCategories.contains(_jobType.toLowerCase());
  }

  List<Map<String, dynamic>> _serviceOptionsForSubscription() {
    final raw = _doc['services'];
    if (raw is! List) return const [];
    return raw
        .whereType<Map>()
        .map((item) => Map<String, dynamic>.from(item))
        .where((item) => (item['service'] ?? '').toString().trim().isNotEmpty)
        .toList();
  }
  ...
  void _openRequestFlow(BuildContext ctx) {
    if (_isCategoryAWorker(w)) {
    ...
    } else if (_isSubscriptionCategoryWorker()) {
      Navigator.push(
        ctx,
        MaterialPageRoute(
          builder: (_) => SubscriptionRequestScreen(
            worker: w,
            services: _serviceOptionsForSubscription(),
          ),
        ),
      );
    } else {
      ScaffoldMessenger.of(ctx).showSnackBar(
        const SnackBar(
          content: Text(
            'Request Service temporarily unavailable for this category.',
          ),
        ),
      );
    }
