// lib/screens/category_c/category_c_request_form_screen.dart
//
// Category C — Subscription / Recurring Jobs (teacher, caregiver)
// Customer fills address, requirement description, preferred schedule,
// then submits → Firestore doc created → CustomerRequestScreen opens.
...
class CategoryCRequestFormScreen extends StatefulWidget {
  final Worker worker;
  const CategoryCRequestFormScreen({super.key, required this.worker});

  @override
  State<CategoryCRequestFormScreen> createState() =>
      _CategoryCRequestFormScreenState();
}

class _CategoryCRequestFormScreenState
    extends State<CategoryCRequestFormScreen> {
  ...
  // Preferred billing cycle hint shown to the worker
  String _preferredCycle = 'monthly';
  ...
  Future<void> _submit() async {
    if (_descCtrl.text.trim().isEmpty) {
      _snack('Please describe your requirement.', isError: true);
      return;
    }
    ...
    try {
      ...
      final docRef = await FirebaseFirestore.instance
          ...
          .add({
            ...
            'categoryType': 'C', // ← Category C flag
            // customer info
            ...
            'preferredBillingCycle': _preferredCycle,
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        Icon(Icons.repeat_rounded, size: 14, color: _C.gradA),
                        SizedBox(width: 8),
                        Expanded(
                          child: Text(
                            'This is a recurring service. After your request is accepted, the worker will send you a subscription plan.',
                            style: TextStyle(
                              fontSize: 11,
                              color: _C.gradA,
                              height: 1.5,
                  ...
                  // ── requirement
                  _sectionLabel('Your Requirement', required: true),
                  const SizedBox(height: 8),
                  _textArea(
                    _descCtrl,
                    hint:
                        'Describe what you need (e.g. Math and Science tuition for Grade 11, 3 sessions per week)…',
                  ),
                  const SizedBox(height: 18),

                  // ── billing cycle preference
                  _sectionLabel('Preferred Billing Cycle'),
                  ...
                  // ── preferred schedule
                  _sectionLabel('Preferred Session Schedule'),
                  const SizedBox(height: 8),
                  _textField(
                    _scheduleCtrl,
                  ...
                  _textArea(
                    _notesCtrl,
                    hint: 'Any extra details for the worker…',
                    lines: 3,
                  ),
  ...
  Widget _cycleToggle(String label, String value) => GestureDetector(
    onTap: () => setState(() => _preferredCycle = value),
    child: AnimatedContainer(
      duration: const Duration(milliseconds: 180),
      padding: const EdgeInsets.symmetric(vertical: 12),
      decoration: BoxDecoration(
        color: _preferredCycle == value
            ? const Color(0xFFF3EEFF)
            : Colors.white,
        borderRadius: BorderRadius.circular(12),
        border: Border.all(
          color: _preferredCycle == value ? _C.gradA : _C.border,
          width: _preferredCycle == value ? 1.5 : 1.0,
        ),
      ),
      child: Center(
        child: Text(
          ...
          style: TextStyle(
            fontSize: 13,
            fontWeight: FontWeight.w600,
            color: _preferredCycle == value ? _C.gradA : _C.txt2,
          ),
        ),
      ),
    ),
